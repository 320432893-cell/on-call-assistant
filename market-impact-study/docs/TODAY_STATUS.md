...
日期：2026-06-13（下方含 2026-06-14、2026-06-16 增补）

## 2026-06-16 增补（收口 + 因果扩展的重大数据更正）

- **收口**：修正 22 家扩展交接（此前谎称"22 家因果已完成"，磁盘实无）；新建项目决策账本 `docs/DECISION_LEDGER.md`；修复被采集改造打断的测试；工作区补齐至通过全部静态闸（`check.py changed` 退出码 0）。
- **⚠️ 数据更正（账本 INV-006）**：项目"异常机构关注"信号依赖的 IR 机构调研数据**只覆盖 2025-06 后**（2025 年原始量较往年跳约 160 倍），是采集近期化造成的人为断层。9 家因果"关注显著（p≈0.001）"头条很可能被夸大，须 caveat（`reports/CAUSAL_RESULTS.md` 已加）；v3/v4 同款特征解释力同受影响。
- **干净长面板重做（账本 INV-007）**：换有真实全覆盖的注意力代理（主：分析师研报强度；稳健性：市场换手异常），22 家重建因果面板 `data/processed/modeling/causal_22/`（n=1034，2017-2026）。结论诚实：会前异常关注→事后相对反应**弱正、两代理方向一致、加控制变强，但不显著**（研报 p≈0.27、换手 p≈0.08）。详见 `reports/CAUSAL_RESULTS_TIER2.md`、代码 `build_causal_panel_22.py`。
- **可选下一步**：22 家面板补 S5 安慰剂 + S6 敏感性；据 INV-006 重述 9 家头条/PPT；课程交付打包（截止 2026-07-03）。
