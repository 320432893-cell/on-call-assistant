...
- 是否先讨论再写入，用户未拍板不落盘。
- 是否优先单文件 + 强读取路径，而不是过早物理解耦。
- 是否把索引写成触发路由器，而不是目录清单或第二份规则正文。
- 是否默认跨文件引用 index，并只由 index 按复杂 / 高风险 / 正式交付 / 副作用条件进入 details。
- 是否让阶段闭包里的 `RULE_ROUTE` 指向总索引或专题 index，而不是 details。
