...
本文件是 `AGENTS.md` § 9 的下放。仅在项目实际多语言配合、或改动跨语言边界时读取，单语言项目不读。

## 1. 跨语言契约

- 跨语言交互点 MUST 有显式契约：HTTP → OpenAPI；RPC → protobuf/gRPC；进程间/文件 → JSON Schema。
- 契约 MUST 单一真相源（SSOT）：定义放一处，各语言**生成**绑定，不手写多份会漂移的类型。
- 跨语言类型映射变化 = 公共契约变化 → 命中 `AGENTS.md` § 6.3 子 agent 审查 + § 4 状态机（契约通常是"依赖"级，不得静默改）。
...
发散选型时的事实底材，非默认推荐（选型走 `AGENTS.md` § 9 + § 5 发散-收敛）：
